...
def part_one():
    ...
    while queue:
        active_spells, player, boss, player_turn = queue.pop(0)
        armor = 0
        ...
        if boss[0] <= 0:
            ...
            for i, spell in enumerate(spells):
                if not active_spells[i] and player[1] >= spells[i][0]:
                    could_act = True
                    ...
                    if new_player[2] < least_mana_spend:
                        print(new_active_spells, new_player, boss, False)
            ...
            if player[0] > 0 and player[2] < least_mana_spend:
                queue.append((active_spells, player, boss, True))
